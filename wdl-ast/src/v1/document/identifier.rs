...
impl TryFrom<Pair<'_, grammar::v1::Rule>> for Identifier {
    ...
    fn try_from(node: Pair<'_, grammar::v1::Rule>) -> Result<Self, Self::Error> {
        match node.as_rule() {
            ...
            node => Err(Error::Common(crate::v1::Error::InvalidNode(format!(
                "identifier cannot be parsed from node type {:?}",
                node
            )))),
        }
    }
...
mod tests {
    ...
    fn it_parses_from_a_supported_node_type() {
        let identifier = macros::test::valid_node!("hello_world", singular_identifier, Identifier);
        assert_eq!(identifier.into_singular().unwrap().as_str(), "hello_world");

        macros::test::valid_node!("hello.there.world", qualified_identifier, Identifier);
    }
