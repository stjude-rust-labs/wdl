...
workflow chip {
    ...
    File? blacklist_ = (if length(blacklists) > 1 then pool_blacklist.ta_pooled else if length(blacklists) > 0 then blacklists[0] else blacklist2_)
    ...
    Int num_rep_nodup_bam = (if length(nodup_bams) < num_rep_bam then num_rep_bam else length(nodup_bams))
    Int num_rep_ta = (if length(tas) < num_rep_nodup_bam then num_rep_nodup_bam else length(tas))
    ...
    Int num_ctl_nodup_bam = (if length(ctl_nodup_bams) < num_ctl_bam then num_ctl_bam else length(ctl_nodup_bams))
    Int num_ctl_ta = (if length(ctl_tas) < num_ctl_nodup_bam then num_ctl_nodup_bam else length(ctl_tas))
    ...
    scatter (i in range(num_rep)) {
        ...
        Boolean paired_end_ = (if !defined(paired_end) && i < length(paired_ends) then paired_ends[i] else select_first([
            paired_end,
        ]))
        ...
        File? ta_xcor = (if defined(bam2ta_no_dedup_R1.ta) then bam2ta_no_dedup_R1.ta else if defined(bam2ta_no_dedup.ta) then bam2ta_no_dedup.ta else ta_)
    ...
    scatter (i in range(num_ctl)) {
        ...
        Boolean ctl_paired_end_ = (if !defined(ctl_paired_end) && i < length(ctl_paired_ends) then ctl_paired_ends[i] else select_first([
            ctl_paired_end,
            paired_end,
        ]))
    ...
    scatter (i in range(num_rep)) {
        ...
        Boolean chosen_ctl_paired_end = (if chosen_ctl_ta_id == -2 then false else if chosen_ctl_ta_id == -1 then ctl_paired_end_[0] else ctl_paired_end_[chosen_ctl_ta_id])

        if (chosen_ctl_ta_id > -2 && chosen_ctl_ta_subsample > 0) {
            call subsample_ctl { input:
                ta = (if chosen_ctl_ta_id == -1 then pool_ta_ctl.ta_pooled else ctl_ta_[chosen_ctl_ta_id]),
    ...
    Array[
        Int] fraglen_tmp = select_all(fraglen_)
...
task read_genome_tsv {
    ...
    output {
        ...
        String? regex_bfilt_peak_chr_name = (if size("regex_bfilt_peak_chr_name") == 0 then "chr[\\dXY]+" else read_string("regex_bfilt_peak_chr_name"))
    }
