//! The format node registry.
//!
//! The format entity registry was introduced only to ensure that all nodes and
//! tokens in the concrete syntax tree have one and _only_ one analogous format
//! entity.
//!
//! The reason this is important to ensure statically is because this assumption
//! of one-to-one mapping between elements within the two types of trees is used
//! within formatting. For example, formatting works by traversing the CST of a
//! WDL document and attempting to cast a node to any format element.
//!
...
//! can map to multiple different formattable elements in different contexts is
...
/// implement them in the future unhindered without introducing breaking changes.
mod private {
...
/// A registry of all known mappings between format elements (individual Rust types
...
trait AstTokenRegistrant: private::SealedToken {
    /// Returns the [`SyntaxKind`]\(s) that can be cast into this AST token type.
    fn register() -> (&'static str, Box<[SyntaxKind]>);
}
