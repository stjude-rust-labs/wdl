...
fn parse<'a, P: Iterator<Item = pest::iterators::Pair<'a, grammar::v1::Rule>>>(
    ...
    PRATT_PARSER
        .map_primary(|node| match node.as_rule() {
            ...
            Rule::string => {
                let inner = dive_one!(node, string_inner, string, Error::Common)?;
...
mod tests {
    ...
    fn ensure_number_works_correctly() {
        let expr = macros::test::valid_node!("1", expression, Expression);
        assert_eq!(
            ensure_number(&expr),
        ...
        let expr = macros::test::valid_node!("1.0", expression, Expression);
        assert_eq!(
            ensure_number(&expr),
        ...
        let expr = macros::test::valid_node!("-+-false", expression, Expression);
        assert_eq!(ensure_number(&expr), None);
    }

    #[test]
    fn it_correctly_parses_floats() {
        let value = macros::test::valid_node!("1.0", expression, Expression);
        assert_eq!(
            value,
            Expression::Literal(Literal::Float(OrderedFloat(1.0)))
        );

        let value = macros::test::valid_node!("1.0e0", expression, Expression);
        assert_eq!(
            value,
            Expression::Literal(Literal::Float(OrderedFloat(1.0)))
        );

        let value = macros::test::valid_node!("1.e0", expression, Expression);
        assert_eq!(
            value,
            Expression::Literal(Literal::Float(OrderedFloat(1.0)))
        );

        let value = macros::test::valid_node!("1e0", expression, Expression);
        assert_eq!(
            value,
            Expression::Literal(Literal::Float(OrderedFloat(1.0)))
        );

        let value = macros::test::valid_node!("1e+0", expression, Expression);
        assert_eq!(
            value,
            Expression::Literal(Literal::Float(OrderedFloat(1.0)))
        );
