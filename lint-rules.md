...
`sprocket doc` is under active development at the time of writing. This tool will auto-document any WDL file in MarkDown. Header-comments found by `sprocket doc` will be re-written in the output `.md` files (without the `##` prefix). Arbitrary and multi-line MarkDown constructions will be preserved adn rendered.

These header-comments should not be used anywhere beside the header.
