...
impl std::fmt::Display for Error {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Error::RuleMismatch(path) => write!(f, "cannot match rule from file: {}", path.display())
        }
    }
}
...
fn inner() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match args.command {
        Command::Parse(args) => {
            let rule = map_rule(&args.rule).map(Ok).unwrap_or_else(|| Err(Error::RuleMismatch(args.path.clone())))?;
...
fn map_rule(rule: &str) -> Option<wdl::Rule> {
    match rule {
        ...
        "workflow_parameter_metadata" => Some(wdl::Rule::workflow_parameter_metadata),
        "workflow_metadata_kv" => Some(wdl::Rule::workflow_metadata_kv),
        "command_heredoc_interpolated_contents" => Some(wdl::Rule::command_heredoc_interpolated_contents),
