...
pub struct Builder {
    ...
    /// Document tasks.
    tasks: IndexMap<Identifier, Task>,
...
impl Builder {
    ...
    /// let document = document::Builder::default()
    ///     .version(document::Version::OneDotOne)?
    ///     .insert_task(name, task)
    ///     .try_build()?;
    /// assert_eq!(document.tasks().len(), 1);
    ///
    /// let (name, task) = document.tasks().first().unwrap();
    ...
    /// # Ok::<(), Box<dyn std::error::Error>>(())
    /// ```
    pub fn insert_task(mut self, name: Identifier, task: Task) -> Self {
