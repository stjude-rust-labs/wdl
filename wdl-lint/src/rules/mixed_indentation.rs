...
fn mixed_indentation(
    ...
    diagnostic = diagnostic.with_rule(ID).with_label(
        format!(
    ...
    if let Some(command_span) = command {
    ...
    diagnostic.with_fix("use either tabs or spaces exclusively for indentation")
}
        ...
        *self = Default::default();
        ...
        if let Some(span) = mixed_span {
            ...
            let in_command_section = self.command_section_spans.iter().any(|cmd_span| {
                span.start() >= cmd_span.start() && span.start() < cmd_span.start() + cmd_span.len()
            });
