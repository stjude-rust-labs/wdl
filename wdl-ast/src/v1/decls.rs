...
impl Type {
    ...
    pub fn children(syntax: &SyntaxNode) -> impl Iterator<Item = Type> {
        syntax.children().filter_map(|c| Type::try_from(c).ok())
    }
}
...
impl TryFrom<SyntaxNode> for Type {
    ...
    fn try_from(syntax: SyntaxNode) -> Result<Self, Self::Error> {
        match syntax.kind() {
            SyntaxKind::MapTypeNode => {
                Ok(Self::Map(MapType::cast(syntax).expect("map type to cast")))
            }
            SyntaxKind::ArrayTypeNode => Ok(Self::Array(
                ArrayType::cast(syntax).expect("array type to cast"),
            )),
            SyntaxKind::PairTypeNode => Ok(Self::Pair(
                PairType::cast(syntax).expect("pair type to cast"),
            )),
            SyntaxKind::ObjectTypeNode => Ok(Self::Object(
                ObjectType::cast(syntax).expect("object type to cast"),
            )),
            SyntaxKind::TypeRefNode => {
                Ok(Self::Ref(TypeRef::cast(syntax).expect("type ref to cast")))
            }
            SyntaxKind::PrimitiveTypeNode => Ok(Self::Primitive(
...
impl Decl {
    ...
    pub fn children(syntax: &SyntaxNode) -> impl Iterator<Item = Decl> {
        syntax.children().filter_map(|c| Decl::try_from(c).ok())
    }
}

impl TryFrom<SyntaxNode> for Decl {
    type Error = ();

    fn try_from(syntax: SyntaxNode) -> Result<Self, Self::Error> {
        match syntax.kind() {
            SyntaxKind::BoundDeclNode => Ok(Self::Bound(
                BoundDecl::cast(syntax).expect("bound decl to cast"),
            )),
            SyntaxKind::UnboundDeclNode => Ok(Self::Unbound(
