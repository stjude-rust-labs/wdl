...
impl Config {
    ...
    pub fn load_or_new(path: PathBuf, version: Version) -> Result<Self> {
        ...
        if log_enabled!(log::Level::Trace) {
            ...
            let num_ignored_errors = result.inner().ignored_errors().len();
    ...
    ///
    /// assert_eq!(config.inner().version(), &core::Version::V1);
    /// assert_eq!(config.inner().repositories().len(), 1);
    /// assert_eq!(config.inner().ignored_errors().len(), 0);
    ///
    ...
    ///
    /// config.inner_mut().extend_ignored_errors(vec![(
    ///     "Foo/Bar:Quux".parse::<gauntlet::document::Identifier>()?,
    ///     String::from("Hello, world!"),
    /// )]);
    ///
    /// assert_eq!(config.inner().version(), &core::Version::V1);
    /// assert_eq!(config.inner().repositories().len(), 2);
    /// assert_eq!(config.inner().ignored_errors().len(), 1);
    ///
    /// Ok::<(), Box<dyn std::error::Error>>(())
    /// ```
