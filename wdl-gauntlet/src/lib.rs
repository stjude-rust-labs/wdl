...
pub struct Args {
    ...
    #[arg(long, global = true)]
    pub silence_error_details: bool,
    ...
    pub specification_version: core::Version,
...
pub async fn gauntlet(args: Args) -> Result<()> {
    ...
    for (index, repository_identifier) in config.inner().repositories().iter().enumerate() {
        ...
        for (path, content) in results {
            ...
            match config.inner().version() {
                core::Version::V1 => {
                    let pt = match grammar::v1::parse(&content) {
                        Ok(tree) => {
                            match tree.warnings() {
                                Some(warnings) => {
                                    trace!(
                    ...
                    let document = match ast::v1::parse(pt) {
    ...
    if args.save_config {
        if !undetected_ignored_errors.is_empty() {
            ...
            config.inner_mut().replace_ignored_errors(
                (&ignored_errors - &undetected_ignored_errors)
                    .union(&unignored_errors)
                    .cloned(),
            );
        }

        config.inner_mut().extend_ignored_errors(
            unignored_errors
                .into_iter()
