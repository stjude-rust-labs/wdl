...
impl TryFrom<Pair<'_, grammar::v1::Rule>> for Import {
    ...
    fn try_from(node: Pair<'_, grammar::v1::Rule>) -> Result<Self, Self::Error> {
        ...
        for node in node.into_inner() {
            match node.as_rule() {
                Rule::import_uri => {
                    let uri = dive_one!(node, string_literal_contents, import_uri, Error::Common)?;
                    builder = builder
                ...
                Rule::import_as => {
                    let identifier_node =
                        dive_one!(node, singular_identifier, import_as, Error::Common)?;
                ...
                Rule::import_alias => {
                    ...
                    let from_node =
                        dive_one!(node.clone(), import_alias_from, import_alias, Error::Common)?;
                    let from = Identifier::try_from(from_node.as_str().to_string())
                        .map_err(Error::Identifier)?;

                    let to_node = dive_one!(node, import_alias_to, import_alias, Error::Common)?;
