...
pub struct Document {
    ...
    /// Document tasks.
    tasks: IndexMap<identifier::singular::Identifier, Task>,
...
impl Document {
    ...
    ///
    /// let import = ast.imports().first().unwrap();
    ...
    ///
    /// let r#struct = ast.structs().first().unwrap();
    ...
    /// let (name, task) = ast.tasks().first().unwrap();
    /// assert_eq!(name.as_str(), "say_hello");
    /// assert_eq!(task.name().as_str(), "say_hello");
    /// assert_eq!(task.command().to_string(), "echo 'Hello, world!'");
    /// ```
    pub fn tasks(&self) -> &IndexMap<identifier::singular::Identifier, Task> {
        &self.tasks
    }
    ...
    /// let ast = ast::v1::parse(pt).unwrap();
    ///
    /// let workflow = ast.workflow().unwrap();
        ...
        for node in node.into_inner() {
            match node.as_rule() {
                ...
                Rule::task => {
                    ...
                    builder = builder.insert_task(task.name().clone(), task);
                }
                Rule::workflow => {
...
mod tests {
    ...
    fn it_parses_from_a_supported_node_type() {
        let version = macros::test::valid_node!("version 1.1", version, Version);
        assert_eq!(version, Version::OneDotOne);
    }

    #[test]
    fn it_fails_to_parse_from_an_unsupported_node_type() {
        macros::test::invalid_node!(
            "version 1.1\n\ntask hello { command <<<>>> }",
            document,
            version,
            Version
