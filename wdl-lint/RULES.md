...
| `CommentWhitespace`         | Clarity, Spacing, Style       | Ensures that comments are properly spaced.                                                          |
...
| `MixedIndentation`          | Clarity, Correctness, Spacing | Ensures consistent indentation (no mixed spaces/tabs) throughout the document, excluding command sections which are handled by the analysis `CommandMixedIndentation` rule. |
