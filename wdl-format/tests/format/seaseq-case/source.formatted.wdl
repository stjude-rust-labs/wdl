...
workflow seaseq {
    ...
    if (!defined(spikein_bowtie_index) && defined(spikein_reference)) {
        ...
        call bowtie.index as spikein_bowtie_idx { input: reference = select_first([spikein_reference, string_spikein]) }
    }
    ...
    if (defined(spikein_bowtie_index)) {
        ...
        if (length(int_spikein_bowtie_index) != 6) {
            # create bowtie index if 6 index files aren't provided
            call bowtie.index as spikein_bowtie_idx_2 { input: reference = select_first([spikein_reference, string_spikein]) }
        }
    }
    ...
    if (multi_fastq) {
        scatter (eachfastq in fastqfiles) {
            ...
            call runspp.runspp as indv_runspp { input: bamfile = select_first([indv_mapping.bklist_bam, indv_mapping.sorted_bam]) }
            call bedtools.bamtobed as indv_bamtobed { input: bamfile = select_first([indv_mapping.bklist_bam, indv_mapping.sorted_bam]) }
    ...
    if (one_fastq) {
        ...
        call runspp.runspp as uno_runspp { input: bamfile = select_first([mapping.bklist_bam, mapping.sorted_bam]) }
        call bedtools.bamtobed as uno_bamtobed { input: bamfile = select_first([mapping.bklist_bam, mapping.sorted_bam]) }
    ...
    call bedtools.bamtobed as forsicerbed { input: bamfile = select_first([merge_markdup.mkdupbam, mapping.mkdup_bam]) }
